...
class SquareFeatures(Transformer):
    ...
    def _transform(self, df):
        for input_col, output_col in zip(self.inputCols, self.outputCols):
            df = df.withColumn(output_col, F.col(input_col) ** 2)
        return df
...
def pipeline(data: DataFrame):
    ...
    # I added this for lab7
    squared_columns = [f"{v}Squared" for v in numeric_features]
    square_features = SquareFeatures(inputCols=numeric_features, outputCols=squared_columns)
    ...
    # Create the pipeline
    # I added square_features here for lab7
    pipeline = Pipeline(stages=[indexer, *imputers_string, imputer_numeric, pairwise_product, square_features, assembler, classifier])
